...
g1(t) = exp(-2d0*abs(t)) - (1d0/a)*((a - abs(t))**2) + (1d0/a)*(exp(-2*a)*t*t)  !"tk = 2*k*a/n - a" 
pi = 4d0*atan(1d0)
n = 64d0 
a = 16d0
G = 0d0 
...
do j = 0, n-1
    do k = 0, n-1 
        t = - a + 2*a*k/n
        G(j) = G(j) + g1(t)*(exp(2d0*pi*im*j*k/n))
    enddo 
...
do j = 0, 512
    ...
    enddo 
  print*, t, g2 + (1d0/a)*((a - abs(t))**2) - (1d0/a)*(exp(-2*a)*t*t) , exp(-2*abs(t))
enddo 
